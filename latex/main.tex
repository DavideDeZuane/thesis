...
\usepackage{xcolor}
\usepackage{listings}
\usepackage{moeptikz} %library for network device
\usepackage{tikz}
\usepackage{comment}
\usepackage{alltt} 
\usepackage{fancyvrb}
...
\usepackage{microtype} %pergestire meglio l'overflow
\usepackage{array} 
\usepackage{multirow}
\usepackage{pgfplots}
\usepackage{dirtree}
...
\lstdefinestyle{yaml}{ 
	...
	keywordstyle=\color{blue}, 
	morekeywords={version,services, cap\_add, volumes, networks, moon:, carol:, build, container\_name, depends\_on},
	numbers=none
}

\lstdefinelanguage{Dockerfile}{ 
	morekeywords={ FROM, RUN, CMD, LABEL, MAINTAINER,
EXPOSE, ENV, ADD, COPY, ENTRYPOINT, VOLUME, USER, WORKDIR, ARG, ONBUILD,
...
\thesistitle{Sviluppo, implementazione e analisi di protocolli quantum-safe per la sicurezza delle comunicazioni satellitari} % Your thesis title, this is used in the title and abstract, print it elsewhere with \ttitle
...
\textbf{SIG} & \textbf{SIG}nature Mechanism \\
